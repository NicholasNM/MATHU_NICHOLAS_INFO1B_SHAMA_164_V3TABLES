...
@app.route("/entreprise_shop_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def entreprise_shop_afficher(id_film_sel):
    print(" entreprise_shop_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                ...
                if not data_shop_entreprise_afficher and id_film_sel == 0:
                    flash("""La table "t_film" est vide. !""", "warning")
                elif not data_shop_entreprise_afficher and id_film_sel > 0:
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            ...
            print("dans edit_genre_film_selected ---> data_shop_all", data_shop_all)
            ...
            # grâce à la variable "id_film_genres_edit_html" dans le fichier "entreprise_shop_afficher.html"
            ...
            data_genre_film_selected, data_shop_entreprise_non_attribues, data_shop_entreprise_attribues = \
            ...
            lst_data_shop_entreprise_non_attribues = [item['id_genre'] for item in data_shop_entreprise_non_attribues]
            ...
            lst_data_shop_entreprise_old_attribues = [item['id_genre'] for item in data_shop_entreprise_attribues]
            ...
            lst_data_shop_entreprise_non_attribues = [item['intitule_genre'] for item in data_shop_entreprise_non_attribues]
            print("lst_all_genres gf_edit_genre_film_selected ", lst_data_shop_entreprise_non_attribues,
                  type(lst_data_shop_entreprise_non_attribues))
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            lst_diff_genres_delete_b = list(set(old_lst_data_shop_entreprise_attribues) -
            ...
            lst_diff_genres_insert_a = list(
                set(new_lst_int_genre_film_old) - set(old_lst_data_shop_entreprise_attribues))
            ...
            strsql_delete_genre_film = """DELETE FROM t_e_shop WHERE fk_shop = %(value_fk_shop)s AND fk_entreprise = %(value_fk_entreprise)s"""
            ...
            with DBconnection() as mconn_bd:
                ...
                for id_genre_ins in lst_diff_genres_insert_a:
                    ...
                    mconn_bd.execute(strsql_insert_shop_entreprise, valeurs_film_sel_genre_sel_dictionnaire)
    ...
    return redirect(url_for('entreprise_shop_afficher', id_film_sel=id_film_selected))
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_entreprise, nom_entreprise, num_entreprise, email_entreprise, GROUP_CONCAT(id_shop) as EntrepriseShop FROM t_e_shop
        ...
        strsql_genres_films_non_attribues = """SELECT id_shop, nom_shop FROM t_shop WHERE id_shop not in(SELECT id_shop as idEntrepriseShop FROM t_e_shop
                                    INNER JOIN t_entreprise ON t_entreprise.id_entreprise = t_e_personnes.fk_entreprise
                                    INNER JOIN t_shop ON t_shop.id_shop = t_e_shop.fk_shop
                                    WHERE id_entreprise = %(value_id_entreprise_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_entreprise, id_shop, nom_entreprise FROM t_e_shop
                                            INNER JOIN t_entreprise ON t_entreprise.id_entreprise = t_e_personnes.fk_entreprise
                                            INNER JOIN t_shop ON t_shop.id_shop = t_e_shop.fk_shop
                                            WHERE id_entreprise = %(value_id_entreprise_selected)s"""
        ...
        with DBconnection() as mc_afficher:
            ...
            print("genres_films_afficher_data ----> data_shop_entreprise_non_attribues ", data_shop_entreprise_non_attribues,
                  " Type : ",
                  type(data_shop_entreprise_non_attribues))
            ...
            return data_film_selected, data_shop_entreprise_non_attribues, data_shop_entreprise_attribues
